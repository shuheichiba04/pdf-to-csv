...
from parsers.fields import parse_all                   # 正規表現で全フィールドを抽出して dict を返す
...
def process_pdf(pdf_path: Path, passwords: list[str], use_cache: bool = False) -> dict:
    ...
    row["text_quality"] = quality["status"]
...
def run_batch(input_dir: Path, passwords: list[str], csv_path: Path):
    ...
    for pdf_path in pdf_files:
        ...
        try:
            ...
            continue
        rows.append(row)
        print(f"    品質: {row['text_quality']}  ページ数: {row['page_count']}")
